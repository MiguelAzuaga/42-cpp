...
void printType(const std::string& label, const Animal& animal)
{
...
int main()
{
	...
	printSeparator("ANIMAL ARRAY CREATION");
	for (int i = 0; i < N; ++i) {
		if (i < N / 2) {
			animals[i] = new Dog();
		} else {
	...
	printSeparator("ANIMAL ARRAY SOUNDS");
	for (int i = 0; i < N; ++i) {
		std::cout << "Animal[" << i << "] (" << animals[i]->getType() << ") sound: ";
		animals[i]->makeSound();
	}
	...
	printSeparator("ANIMAL ARRAY DELETION");
	for (int i = 0; i < N; ++i) {
		delete animals[i];
	}
